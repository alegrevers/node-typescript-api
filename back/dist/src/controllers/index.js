    ...
    sendCreateUpdateErrorResponse(res, error) {
        if (error instanceof mongoose_1.default.Error.ValidationError) {
            ...
            res
                ...
                .send(api_error_1.default.format({ code: clientErrors.code, message: clientErrors.error }));
        }
        else {
            logger_1.default.error(error);
            res.status(500).send(api_error_1.default.format({ code: 500, message: 'Something went wrong!' }));
        }
    }
    handleClientErrors(error) {
        const duplicatedKindErrors = Object.values(error.errors)
            .filter((err) => err.kind === user_1.CUSTOM_VALIDATION.DUPLICATED);
